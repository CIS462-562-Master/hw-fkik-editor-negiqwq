...
ATransform ATransform::Inverse() const
{
	...
	vec3 inv_mTrans = m_translation * -1;
	mat3 inv_mRot = m_rotation.Transpose();
	return ATransform(inv_mRot, inv_mTrans);
}
...
vec3 ATransform::RotTrans(const vec3& vecToTransform) const
{
	...
	return vec3(m_rotation * vecToTransform + m_translation);
}
...
ATransform operator * (const ATransform& H1, const ATransform& H2)
{
	...
	vec3 comb_mTrans = H1.m_rotation * H2.m_translation + H1.m_translation;
	mat3 comb_mRot = H1.m_rotation * H2.m_rotation;
	return ATransform(comb_mRot, comb_mTrans);
}
