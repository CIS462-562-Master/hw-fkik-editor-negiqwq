...
void quat::FromRotation(const mat3& rot)
{
	...
	mQ[VW] = sqrt(1 + rot[0][0] + rot[1][1] + rot[2][2]) / 2;
...
quat quat::Slerp(const quat& q0, const quat& q1, double u)
{
	...
	double rotAngle = acos(Dot(q0, q1));
	q = (sin((1 - u) * rotAngle) / sin(rotAngle)) * q0 + (sin(u * rotAngle) / sin(rotAngle)) * q1;
...
mat3 quat::ToRotation () const
{
	...
	m[0][0] = 1 - 2 * (mQ[VY] * mQ[VY] + mQ[VZ] * mQ[VZ]);
	m[0][1] = 2 * (mQ[VX] * mQ[VY] - mQ[VZ] * mQ[VW]);
	m[0][2] = 2 * (mQ[VX] * mQ[VZ] + mQ[VY] * mQ[VW]);
	m[1][0] = 2 * (mQ[VX] * mQ[VY] + mQ[VZ] * mQ[VW]);
	m[1][1] = 1 - 2 * (mQ[VX] * mQ[VX] + mQ[VZ] * mQ[VZ]);
	m[1][2] = 2 * (mQ[VY] * mQ[VZ] - mQ[VX] * mQ[VW]);
	m[2][0] = 2 * (mQ[VX] * mQ[VZ] - mQ[VY] * mQ[VW]);
	m[2][1] = 2 * (mQ[VY] * mQ[VZ] + mQ[VX] * mQ[VW]);
	m[2][2] = 1 - 2 * (mQ[VX] * mQ[VX] + mQ[VY] * mQ[VY]);
