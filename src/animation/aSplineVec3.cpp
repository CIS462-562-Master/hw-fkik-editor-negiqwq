    ...
    int segment, double u)
{
	...
	vec3 b12 = b1 * (1 - u) + b2 * u;
	vec3 b23 = b2 * (1 - u) + b3 * u;
	vec3 b012 = b01 * (1 - u) + b12 * u;
	vec3 b123 = b12 * (1 - u) + b23 * u;
	curveValue = b012 * (1 - u) + b123 * u;
    ...
    int segment, double u)
{
	...
	Eigen::Vector4d uMat(1, u, pow(u, 2), pow(u, 3));
	mMat << 1, -3, 3, -1,
		    0, 3, -6, 3,
			0, 0, 3, -3,
			0, 0, 0, 1;
	mMat = mMat * uMat;
	curveValue = b0 * mMat(0, 0) + b1 * mMat(1, 0) + b2 * mMat(2, 0) + b3 * mMat(3, 0);
    ...
    vec3& startPoint, vec3& endPoint)
{
    ...
    for (int i = 1; i < keys.size(); i++)
    {
		...
		b3 = keys[i].second;
		if (i == 1) {
			b1 = b0 + (keys[i].second - keys[i - 1].second) / 3;
		} else {
			b1 = b0 + (keys[i].second - keys[i - 2].second) / 6;
		}
		if (i == keys.size() - 1) {
			b2 = b3 - (keys[i].second - keys[i - 1].second) / 3;
		} else {
			b2 = b3 - (keys[i + 1].second - keys[i - 1].second) / 6;
		}
    ...
    vec3& startPoint, vec3& endPoint)
{
	...
	Eigen::MatrixXd aMat(len, len);

	// Initializing A for natural endpoint conditions
	for (int i = 0; i < len; i++) {
		for (int j = 0; j < len; j++) {
			if (i == 0) {
				if (j == 0) {
	...
	for (int i = 0; i < len; i++) {
		...
		for (int j = 0; j < len; j++) {
			if (j == 0) {
				ctrlPtI += aMat(i, j) * 3 * (keys[1].second - keys[0].second);
			}
			else if (j == len - 1) {
				ctrlPtI += aMat(i, j) * 3 * (keys[j].second - keys[j - 1].second);
			}
			else {
				ctrlPtI += aMat(i, j) * 3 * (keys[j + 1].second - keys[j - 1].second);
			}
		}
		ctrlPoints[i] = ctrlPtI;
	}
}
	...
	int segment, double u)
{
	...
	// Converting the angles to find the shortest path for interpolation
	for (int i = 0; i < 3; i++) {
		while (abs(key0[i] - key1[i]) > 180) {
			if (key0[i] > key1[i]) {
				key1[i] += 360;
			}
			else {
	...
	std::vector<vec3>& ctrlPoints, vec3 & startPoint, vec3 & endPoint)
{
	...
	// Converting the angles to find the shortest path for interpolation
	std::vector<vec3> convKeys;
	for (int i = 0; i < keys.size(); i++) {
		convKeys.push_back(keys[i].second);
	}
	for (int i = 0; i < convKeys.size() - 1; i++) {
		for (int j = 0; j < 3; j++) {
			while (abs(convKeys[i][j] - convKeys[i + 1][j]) > 180) {
				if (convKeys[i][j] > convKeys[i + 1][j]) {
					convKeys[i + 1][j] += 360;
				}
				else {
					convKeys[i + 1][j] -= 360;
	...
	// Computing control points based on the converted angles
	for (int i = 1; i < keys.size(); i++)
	{
		vec3 b0, b1, b2, b3;
		...
		b3 = convKeys[i];
		if (i == 1) {
			b1 = b0 + (convKeys[i] - startPoint) / 6;
		}
		else {
			b1 = b0 + (convKeys[i] - convKeys[i - 2]) / 6;
		}
		if (i == keys.size() - 1) {
			b2 = b3 - (endPoint - convKeys[i - 1]) / 6;
		}
		else {
			b2 = b3 - (convKeys[i + 1] - convKeys[i - 1]) / 6;
		}
